...
";
    ...
    ""response"": {
        ...
        {
            var tariffHelper = new TeslaRatePlanTariffProvider((JsonObject)JsonObject.Parse(ratePlanJsonWithSeasons), 0.0m, 0.0m);
        ...
        public async Task GetWeekDayTariffsForNonWrappingSeasonWithInvalidEndDate()  // Weird, but telemetry shows this can happen.
        {
            ...
            var tariffHelper = new TeslaRatePlanTariffProvider(ratePlan, 0.0m, 0.0m);
        ...
        {
            var tariffHelper = new TeslaRatePlanTariffProvider((JsonObject)JsonObject.Parse(ratePlanJsonSingleRate), 0.0m, 0.0m);
            var tariffs = await tariffHelper.GetTariffsForDay(new DateTime(2024, 6, 5, 0, 0, 0));
        ...
        {
            var tariffHelper = new TeslaRatePlanTariffProvider((JsonObject)JsonObject.Parse(ratePlanJsonWithSeasons), 1.11m, 0.0m);
            var energyHistory = (JsonArray)JsonArray.Parse(energyHistoryJson);
            var rates = await tariffHelper.GetEnergyCostAndFeedInFromEnergyHistory(energyHistory.ToList());
