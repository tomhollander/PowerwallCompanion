...
namespace TeslaAuth
{
    public static class TeslaAuthHelper
    {
        ...
        private static LoginInfo InitializeLogin()
        {
            ...
            result.CodeChallenge = Convert.ToBase64String(Encoding.UTF8.GetBytes(code_challenge_SHA256));
            ...
            using (HttpClientHandler ch = new HttpClientHandler())
            {
                ...
                using (HttpClient client = new HttpClient(ch))
                {
                    ...
                    using (FormUrlEncodedContent content = new FormUrlEncodedContent(formFields))
                    {
                        ...
                        {
                            { "client_id", "ownerapi" },
                            {"code_challenge", loginInfo.CodeChallenge },
                            {"code_challenge_method", "S256" },
                            {"redirect_uri", "https://auth.tesla.com/void/callback" },
                            {"response_type","code" },
                            {"scope","openid email offline_access" },
                        ...
                        using (var queryContent = new FormUrlEncodedContent(values))
                        {
                            b.Query = queryContent.ReadAsStringAsync().Result;
                        }
                        ...
                        var decoder = new WwwFormUrlDecoder(location.Query);
